...
if ($usp_options['logged_in_users'] && !is_user_logged_in()) :
	...
	<div id="user-submitted-posts">
		...
		<form id="usp_form" method="post" enctype="multipart/form-data" action="">
			<div id="usp-error-message" class="usp-callout-failure usp-hidden"><?php esc_html_e('Please complete the required fields.', 'usp'); ?>
			</div>
			<?php echo usp_error_message();
				...
				if (isset($_GET['success']) && $_GET['success'] == '1') :
					...
					<fieldset class="usp-name">
						...
						<input id="user-submitted-name" name="user-submitted-name" type="text" value="" <?php if (usp_check_required('usp_name')) {
																														echo $usp_required;
																													} ?> class="usp-input">
					</fieldset>
				<?php }
					...
					<fieldset class="usp-email">
						...
						<input id="user-submitted-email" name="user-submitted-email" type="text" value="" <?php if (usp_check_required('usp_email')) {
																															echo $usp_required;
																														} ?> class="usp-input">
					</fieldset>
				<?php }
					...
					<fieldset class="usp-title">
						...
						<input id="user-submitted-title" name="user-submitted-title" type="text" value="" <?php if (usp_check_required('usp_title')) {
																															echo $usp_required;
																														} ?> class="usp-input">
					</fieldset>
				<?php }
					...
					<fieldset class="usp-tags">
						...
						<input id="user-submitted-tags" name="user-submitted-tags" type="text" value="" <?php if (usp_check_required('usp_tags')) {
																														echo $usp_required;
																													} ?> class="usp-input">
					</fieldset>
				<?php }
				...
				<?php }
						if (($usp_options['usp_url'] == 'show' || $usp_options['usp_url'] == 'optn') && ($usp_display_url)) { ?>

					<fieldset class="usp-url">
						<label for="user-submitted-url"><?php esc_html_e('URL van het bericht', 'usp'); ?></label>
						<input id="user-submitted-url" name="user-submitted-url" type="text" value="" <?php if (usp_check_required('usp_url')) {
																														echo $usp_required;
																													} ?> class="usp-input">
					</fieldset>
				...
				<?php }
					...
					<fieldset class="usp-category">
						...
						<select id="user-submitted-category" name="user-submitted-category" <?php if (usp_check_required('usp_category')) {
																											echo $usp_required;
																										} ?> class="usp-select">
							<option value=""><?php esc_html_e('Please select a category..', 'usp'); ?>
							</option>
							<?php foreach ($usp_options['categories'] as $categoryId) {
											$category = get_category($categoryId);
											if (!$category) {
												continue;
											} ?>

								<option value="<?php echo $categoryId; ?>"><?php $category = get_category($categoryId);
				...
				<?php }
						if ($usp_options['custom_field'] == 'show' || $usp_options['custom_field'] == 'optn') { ?>

					<fieldset class="usp-custom">
						<label for="user-submitted-custom"><?php echo esc_html($usp_custom_label); ?></label>
						<textarea id="user-submitted-custom" name="<?php echo esc_attr($usp_custom_name); ?>" rows="5" <?php if (usp_check_required('usp_content')) {
																																		echo $usp_required;
					...
					<?php }
						...
						<fieldset class="usp-content">
							...
							<?php } else { ?>
								...
								<textarea id="user-submitted-content" name="user-submitted-content" rows="5" <?php if (usp_check_required('usp_content')) {
																																	echo $usp_required;
																																} ?> class="usp-textarea"></textarea>
							<?php } ?>
					...
					<?php }
							if ($usp_recaptcha_public && $usp_recaptcha_private && $usp_options['usp_recaptcha'] == 'show') { ?>
					...
					<?php }
								...
								<div id="user-submitted-image">
									<?php // upload files
													...
													if (empty($usp_addAnother)) {
														$usp_addAnother = '<a href="#" id="usp_add-another" class="usp-no-js">' . esc_html__('Add another image', 'usp') . '</a>';
													}
										...
										<?php endfor; ?>
										<?php if ($usp_minImages < $usp_maxImages) : echo $usp_addAnother;
															endif; ?>
									<?php else : ?>
						...
						<fieldset class="usp-captcha">
							...
							<input id="user-submitted-captcha" name="user-submitted-captcha" type="text" value="" <?php echo $usp_required; ?> class="usp-input<?php echo $usp_captcha; ?>" data-parsley-excluded="true">
						</fieldset>
					<?php } ?>
					...
					<div id="usp-submit">
						...
						<input type="submit" class="usp-submit" id="user-submitted-post" name="user-submitted-post" value="<?php esc_attr_e('Plaats bericht', 'usp'); ?>">
